<script lang="ts">
  ...
  export let checkedAddonIds: Set<string> = new Set();
  export let onToggle: (id: string) => void = () => {};

  // Filter addons based on selected package
  const filteredAddons = $derived(() => {
    if (!selectedPkgId) return [];
    return addons.filter((a) => !a.pkgs || a.pkgs.includes(selectedPkgId!));
  });
</script>
...
<section class="addons">
  <div class="container--narrow">
    ...
    {:else}
          ...
          <li class="addons__item-wrapper">
            ...
            >
              ...
              <div class="addons__info">
                ...
                {#if addon.note}
                  <span class="addons__note">— {addon.note}</span>
                {/if}
              </div>
      ...
      <p class="addons__footer-note label">
        All add-ons must be requested at least 72 hours in advance.
      </p>
    {/if}
  </div>
</section>
